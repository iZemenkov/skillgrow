...
# --------- CLI ---------
def parse_args():
    p = argparse.ArgumentParser(description="Extract skills and build embeddings")
    # новые имена + алиасы на старые
    p.add_argument("--input", "--src", dest="src",
                   default="data/raw_data/vacancies_master.csv",
                   help="Входной CSV (master)")
    p.add_argument("--out-skills", dest="out_skills",
                   default="data/processed/extracted_skills.csv",
                   help="Куда сохранить извлечённые навыки")
    p.add_argument("--out-emb", dest="out_emb",
                   default="data/processed/embeddings.parquet",
                   help="Куда сохранить эмбеддинги")
    p.add_argument("--model-id", dest="model_id",
                   default="ai-forever/sbert_large_nlu_ru",
                   help="HF id модели для эмбеддингов")
    p.add_argument("--max-rows", type=int, default=None,
                   help="Ограничить число строк (для отладки)")
...
if __name__ == "__main__":
    args = parse_args()
    Path(args.out_skills).parent.mkdir(parents=True, exist_ok=True)
